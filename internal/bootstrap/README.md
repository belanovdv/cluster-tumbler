...
The initial `desired` document seeded for each management group is `state=passive, managed=false`. This means a freshly joined node is passive and outside controller authority until an operator explicitly enables it. The first node to write wins (`TryPutIfAbsent`); subsequent agents do not overwrite an already-managed group.
